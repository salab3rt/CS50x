...
//improved total runtime to sub 0.2 sec, by using a 3D array[56][46]
...
bool load(const char *dictionary)
{
    ...
    while (fscanf(dict, "%s", tmp) != EOF)  //scan file for trings and store them in tmp variable
    {
        ...
        unsigned int index1 = strlen(tmp);

        strcpy(new_node->word, tmp); //copy each word to new node node

        new_node->next = table[index0][index1]; //new node points at where table is pointing
        table[index0][index1] = new_node;    //then point table at new node
        word_counter++; //count each word copied to table

        //printf("table[%i][%i] - %s \n", index0, index1, table[index0][index1]->word);
    }
...
bool unload(void)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            for (trav = table[i][k]; trav != NULL; trav = tmp) //traverse the array using a traversal variable
            {
                tmp = trav->next;   //hold trav->next pointer in a temporary variable
                free(trav);         //free traversal variable, and subsequently free the position that table[i][k] was pointing at
